...
unordered_map<string, unsigned int> make_dictionary(vector<string> words){
    unordered_map<string, unsigned int> dictionary;
    for(string w : words){
        auto it = dictionary.find(w);
        if(it == dictionary.end()){
            dictionary.insert(make_pair(w,1));
        }else{
            it->second++;
        }
    }
...
void checkMagazine(vector<string> magazine, vector<string> note) {
    unordered_map<string, unsigned int> mapMagazine = make_dictionary(magazine);
    for(string word : note){
        auto it = mapMagazine.find(word);
        if(it==mapMagazine.end() || it->second == 0){
            cout << "No\n";
            return;
        }else{
            it->second--;

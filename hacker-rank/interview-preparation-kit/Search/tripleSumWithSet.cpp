...
long triplets(vector<int> a, vector<int> b, vector<int> c) {
    ...
    long answer = 0;
    long as = 0, cs = 0;
    auto a_it = a_set.begin();
    auto c_it = c_set.begin();
    for(auto b_it = b_set.begin(); b_it != b_set.end(); b_it++){
        for(; a_it != a_set.end() && *a_it <= *b_it; a_it++){
            as++;
        }
        for(; c_it != c_set.end() && *c_it <= *b_it; c_it++){
            cs++;
        }
